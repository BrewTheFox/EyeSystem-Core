...
from formularios import form_reconocimiento
...
def abrir_ventana_toma_de_datos(frame_principal, regresar_callback):
    ...
    def iniciar_entrenamiento():
        ...
        def entrenamiento_thread():
            try:
                # Calcular el intervalo para que dure exactamente 6 segundos
                DURACION_TOTAL = 4000  # 6 segundos en milisegundos
                PASOS = 100
                INTERVALO = DURACION_TOTAL / PASOS  # 60ms por paso
                
                # Simular progreso
                for i in range(PASOS):
                    ventana_carga.after(int(INTERVALO * i), lambda v=i/PASOS: ventana_carga.actualizar_progreso(v))
                
                entrenamiento.ejecutar_entrenamiento()
                ventana_carga.after(DURACION_TOTAL, ventana_carga.destroy)
